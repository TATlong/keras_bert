...
class ScaledDotProductAttention(Layer):
    ...
    注意力层的计算：\text{Attention}(Q, K, V) = \text{softmax}(\frac{Q K^T}{\sqrt{d_k}}) V
    ...
    def call(self, inputs, mask=None, **kwargs):
        ...
        # Attention(Q,K,V)计算公式在这里开始
        e = K.batch_dot(query, key, axes=2) / K.sqrt(K.cast(feature_dim, dtype=K.floatx()))
        e = K.exp(e - K.max(e, axis=-1, keepdims=True))    # softmax计算

        # 这里是encoder和decoder的区别，是否只使用历史时刻的token
