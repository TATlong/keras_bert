# -*- coding: UTF-8 -*-
from bertTAT.transformer_utils.scale_dot_product_attention import ScaledDotProductAttention
...
class MultiHeadAttention(Layer):
    ...
    def call(self, inputs, mask=None):
        ...
        if isinstance(mask, list):
            ...
            q_mask = k_mask = v_mask = mask
        # Wq、Wk、Wv分别是映射矩阵，输入的q=k=v是相同的，即是embedding+positional的vector
        q = K.dot(q, self.Wq)
        k = K.dot(k, self.Wk)
        v = K.dot(v, self.Wv)
        # 判断是否进行一层空间变换
        ...
        # 使用Wo映射矩阵对concatenate起来的多头输出序列进行降维
        y = K.dot(y, self.Wo)
        if self.use_bias:
            y += self.bo
        if self.activation is not None:
